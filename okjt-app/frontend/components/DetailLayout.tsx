...
export default function DetailLayout({
    ...
    return (
        <main className="flex min-h-screen flex-col bg-background">
            ...
            >
                <div className="flex flex-wrap items-center gap-8 pt-8 border-t border-border/50 mt-8">
                    {significantFigure && (
                        <div className="flex flex-col">
                            <span className="text-[10px] text-muted-foreground/50 uppercase tracking-widest mb-1">{significantFigureLabel}</span>
                            <span className="text-2xl font-mono text-primary font-bold">{significantFigure}</span>
                        </div>
                    )}
                    {category && (
                        <div className="flex flex-col">
                            <span className="text-[10px] text-muted-foreground/50 uppercase tracking-widest mb-1">{categoryLabel}</span>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                        <div className="lg:col-span-8 space-y-16">
                            ...
                            {description && (
                                <motion.div 
                                    ...
                                    viewport={{ once: true }}
                                    className="p-8 border-l-2 border-primary bg-primary/5 rounded-r-xl"
                                >
                                    <div 
                                        className="text-xl md:text-2xl font-light text-foreground/90 leading-relaxed italic prose prose-invert max-w-none prose-p:italic prose-p:text-foreground"
                            ...
                            <div className="space-y-20">
                                {challengeHtml && (
                                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                                        ...
                                        <div 
                                            className="text-muted-foreground leading-relaxed text-lg font-light prose dark:prose-invert max-w-none prose-p:leading-relaxed prose-p:text-muted-foreground prose-strong:text-foreground"
                        ...
                        <div className="lg:col-span-4">
                            <div className="sticky top-32 space-y-8">
                                <div className="p-8 bg-card border border-border/50 rounded-2xl shadow-sm">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-8">
                                        {technologies && Array.isArray(technologies) ? (
                                            technologies.map((tech: string, i: number) => (
                                                <span key={i} className="px-3 py-1 bg-secondary text-primary text-[10px] font-bold uppercase tracking-widest border border-primary/10">
                                                    {tech}
                                                </span>
                                            ))
                                        ) : (
                                            <span className="text-muted-foreground text-xs italic">{fallbackStackText}</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="space-y-4 pt-6 border-t border-border/50">
                                        ...
                                        <Button asChild variant="outline" className="w-full h-12 gap-2 font-bold uppercase tracking-widest text-[10px] rounded-none">
                                ...
                                {/* Focus / DNA List */}
                                <div className="p-8 bg-secondary/5 border border-border/30 rounded-2xl">
                                    <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4">{focusAreasTitle}</h4>
            ...
            {testimonialQuote && (
                <section className="py-24 bg-secondary/5 relative overflow-hidden">
                    ...
                    <div className="max-w-[800px] mx-auto px-6 relative z-10 text-center">
                        ...
                        <div 
                            className="text-2xl md:text-3xl font-light text-foreground leading-relaxed italic mb-8 prose prose-invert max-w-none prose-p:italic prose-p:text-foreground"
                            dangerouslySetInnerHTML={{ __html: testimonialQuote }}
                        />
                        <div>
                            <div className="text-foreground font-bold">{testimonialAuthor}</div>
                            <div className="text-primary text-xs uppercase tracking-widest mt-1">{testimonialLabel}</div>
                        </div>
                    </div>
                </section>
            )}
                ...
                <section className="py-24 bg-background">
                    <div className="max-w-[1200px] mx-auto px-6">
                        <div className="flex flex-col items-center mb-16">
                            <h2 className="text-[10px] font-bold text-primary uppercase tracking-[0.3em] mb-4 flex items-center gap-3">
                                <span className="w-8 h-[1px] bg-primary/30" /> {galleryTagline}
                            </h2>
                            <h3 className="text-3xl font-bold text-foreground">{galleryTitle}</h3>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {gallery.map((img: string, i: number) => (
                                <motion.div 
                                    ...
                                    className={`relative aspect-video overflow-hidden border border-border/50 group h-full ${i === 0 ? 'md:col-span-2' : ''}`}
                                >
                                    <Image 
                                        src={img} 
                ...
                <section className="py-24 bg-secondary/5 border-y border-border/50">
                    <div className="max-w-[1200px] mx-auto px-6">
                        <div className="flex items-center justify-between mb-12">
                            <h2 className="text-2xl md:text-3xl font-bold text-foreground">{relatedTitle}</h2>
                            <Link href={relatedAllLink} className="text-primary text-sm font-bold uppercase tracking-widest hover:underline flex items-center gap-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            {relatedItems.map((item: any) => {
                                ...
                                return (
                                    <Link key={item.id} href={`${relatedLinkPrefix}/${item.slug}`} className="group block relative aspect-[21/9] overflow-hidden border border-border/50 bg-card">
                                        {imgSrc && (
                                        ...
                                        <div className="absolute inset-0 p-8 flex flex-col justify-end bg-gradient-to-t from-background via-background/40 to-transparent">
                                            <span className="text-primary font-bold text-[10px] uppercase tracking-widest mb-2">{taglineText}</span>
