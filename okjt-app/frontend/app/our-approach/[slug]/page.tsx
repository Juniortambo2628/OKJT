...
export default function ProductPage() {
    ...
    // Get hero media from settings override, then from pillar object, finally fallback
    const heroMedia = React.useMemo(() => {
        if (!pillar) return ''
        const key = `hero_pillar_${pillar.slug?.replace(/-/g, '_')}`
        return getSetting(key) || pillar.image || '/NI-Digital-Assets/financial-technology.jpg'
    }, [pillar, getSetting])
